...
AcceptorSocketInfo : Type
AcceptorSocketInfo = Ptr
...
data TCPServerRes : TCPServerStep -> Type where
  TCPServerStateAllocated : AcceptorSocketInfo -> TCPServerRes t
  TCPServerError : (Maybe AcceptorSocketInfo) -> TCPServerRes t
...
instance Handler TcpServer IO where
  ...
  handle () (AllocateState) k = do
    ...
    k (TCPServerStateAllocated ptr) True
  -- Deallocation
  handle (TCPServerError (Just ptr)) (FreeState) k = do
    mkForeign (FFun "idrnet_deallocate_conn_info" [FPtr] FUnit) ptr
    k () ()

  handle (TCPServerError Nothing) (FreeState) k = k () ()

  handle (TCPServerStateAllocated ptr) (FreeState) k = do
    mkForeign (FFun "idrnet_deallocate_conn_info" [FPtr] FUnit) ptr
    k () ()
  ...
  -- Sending Data
  --sendData : Client -> String -> Eff IO [TCPSERVER (TCPServerRes Listening)] (SocketOperationRes ())
  handle (TCPServerStateAllocated ptr) (SendData c s) k = do
